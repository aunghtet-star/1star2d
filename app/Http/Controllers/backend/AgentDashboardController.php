...
class AgentDashboardController extends Controller
{
    public function index()
    {
        ...
        return view('backend.agent-dashboard.index');
    }

    public function ssd()
    {
        $users = AdminUser::role('Agent')->get();
        ...
        return Datatables::of($users)
            ...
            ->addColumn('action', function ($each) {
                $edit_icon = '<a href="'.url('/admin/agent-dashboard/'.$each->id.'/edit').'" class="text-warning"><i class="fas fa-user-edit"></i></a>';
                //$delete_icon = '<a href="'.url('/admin/agent-dashboard/'.$each->id).'" data-id="'.$each->id.'" class="text-danger" id="delete"><i class="fas fa-trash"></i></a>';
                $detail_icon = '<a href="'.url('/admin/agent-dashboard/'.$each->id).'"  data-id="'.$each->id.'" id="show" onclick="show()" ><i class="fas fa-eye"></i></a>';

                $add_icon = '<a href="'.url('admin/agent-dashboard/'.$each->id.'/add').'" class="text-success"><i class="fas fa-circle-plus"></i></a>';

                $substract_icon = '<a href="'.url('admin/agent-dashboard/'.$each->id.'/subtract').'" class="text-danger" ><i class="fas fa-circle-minus"></i></a>';
    ...
    public function edit($id)
    {
        ...
        return view('backend.agent-dashboard.edit', compact('user'));
    }
    ...
    public function update(UpdateUserDashboard $request, $id)
    {
        ...
        return redirect('admin/agent-dashboard')->with('update', 'Updated Successfully');
    ...
    public function show($id, Request $request)
    {
        ...
        //Transaction History
        $user_transactions = WalletHistory::where('type','agent')->where('user_id', $id)->whereDate('created_at', $date. ' '.'00:00:00')->orderBy('created_at', 'DESC')->get();

        $fix_money_from_admins = FixMoneyFromAdmin::where('user_id',$id)->whereDate('created_at', $date. ' '.'00:00:00')->orderBy('created_at', 'DESC')->get();
        return view('backend.agent-dashboard.detail', compact('user', 'user_wallet', 'user_transactions','fix_money_from_admins'));
    }
    ...
    public function add($id)
    {
        ...
        return view('backend.agent-dashboard.add', compact('user'));
...
//            Notification::send([$to_account], new AddAndWithdraw($title, $message, $transaction_id,$sourceable_id, $sourceable_type));
            DB::commit();
            return redirect('admin/agent-dashboard')->with('create', 'Added successfully');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect('admin/agent-dashboard')->withErrors(['fail' => 'Something wrong'])->withInput();
        }
    }
    ...
    public function subtract($id)
    {
        ...
        $user = AdminUser::find($id);
        return view('backend.agent-dashboard.subtract', compact('user'));
...
//            Notification::send([$to_account], new AddAndWithdraw($title, $message, $transaction_id,$sourceable_id, $sourceable_type));
            ...
            return redirect('admin/agent-dashboard')->with('create', 'Remove successfully');
        } catch (\Exception $e) {

            DB::rollBack();

            return redirect('admin/agent-dashboard')->withErrors(['fail' => 'Something wrong'. $e->getMessage()])->withInput();
        }
    }
}
