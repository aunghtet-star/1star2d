...
class TwoController extends Controller
{
    ...
    public function twoHistoryAM(Request $request)
    {
        ...
        $two_transactions = Two::select('two', DB::raw('SUM(amount) as total'))->groupBy('two')->groupBy('two')->where('admin_user_id', Auth::guard('adminuser')->user()->id)->whereBetween('created_at', [Carbon::parse($date.' '.'00:00:00'),Carbon::parse($date.' '.'11:59:00')])->get();
        ...
        foreach($two_transactions as $two_transaction){
            ...
            $two_overviews->date = now()->format('Y-m-d');
            $two_overviews->save();
            }
        }

        $two_overviews_am = TwoOverview::where('admin_user_id', Auth::guard('adminuser')->user()->id)->whereBetween('created_at', [Carbon::parse($date.' '.'00:00:00'),Carbon::parse($date.' '.'11:59:00')])->get();

        $two_transactions_total = TwoOverview::select('amount')->where('admin_user_id', Auth::guard('adminuser')->user()->id)->whereBetween('created_at', [Carbon::parse($date.' '.'00:00:00'),Carbon::parse($date.' '.'11:59:00')])->sum('amount');
        $new_amount_total = TwoOverview::select('new_amount')->where('admin_user_id', Auth::guard('adminuser')->user()->id)->whereBetween('created_at', [Carbon::parse($date.' '.'00:00:00'),Carbon::parse($date.' '.'11:59:00')])->sum('new_amount');
    ...
    public function twoHistoryPM(Request $request)
    {
        ...
        $two_transactions = Two::select('two', DB::raw('SUM(amount) as total'))->groupBy('two')->groupBy('two')->where('admin_user_id', Auth::guard('adminuser')->user()->id)->whereBetween('created_at', [Carbon::parse($date.' '.'12:00:00'),Carbon::parse($date.' '.'23:59:00')])->get();
        ...
        $two_overviews_pm = TwoOverview::where('admin_user_id', Auth::guard('adminuser')->user()->id)->whereBetween('created_at', [Carbon::parse($date.' '.'11:59:00'),Carbon::parse($date.' '.'23:59:00')])->get();

        $two_transactions_total = TwoOverview::select('amount')->where('admin_user_id', Auth::guard('adminuser')->user()->id)->whereBetween('created_at', [Carbon::parse($date.' '.'12:00:00'),Carbon::parse($date.' '.'23:59:00')])->sum('amount');
        $new_amount_total = TwoOverview::select('new_amount')->where('admin_user_id', Auth::guard('adminuser')->user()->id)->whereBetween('created_at', [Carbon::parse($date.' '.'12:00:00'),Carbon::parse($date.' '.'23:59:00')])->sum('new_amount');
    ...
    {
        $date = $request->date ?? now()->format('Y-m-d');
        $two_transactions = TwoOverview::where('admin_user_id', Auth::guard('adminuser')->user()->id)->whereBetween('created_at', [Carbon::parse($date.' '.'00:00:00'),Carbon::parse($date.' '.'11:59:00')])->get();
        $two_transactions_total = TwoOverview::select('amount')->where('admin_user_id', Auth::guard('adminuser')->user()->id)->whereBetween('created_at', [Carbon::parse($date.' '.'00:00:00'),Carbon::parse($date.' '.'11:59:00')])->sum('amount');
    ...
    public function twoKyonPM(Request $request)
    {
        ...
        $two_transactions = TwoOverview::where('admin_user_id', Auth::guard('adminuser')->user()->id)->whereBetween('created_at', [Carbon::parse($date.' '.'11:59:00'),Carbon::parse($date.' '.'23:59:00')])->get();
        $two_transactions_total = TwoOverview::select('amount')->where('admin_user_id', Auth::guard('adminuser')->user()->id)->whereBetween('created_at', [Carbon::parse($date.' '.'12:00:00'),Carbon::parse($date.' '.'23:59:00')])->sum('amount');
